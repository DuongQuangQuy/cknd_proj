            ...
            if (this.view.arch.tag === 'tree') {
                return ListRenderer.extend({
                    ...
                    _onRowClicked: function(ev) {
                        // Prevent multiple clicks while form is opening/open
                        if (this._isFormOpen) {
                            return;
                        }
                        ...
                        // Nếu click vào checkbox, không mở form
                        if ($(ev.target).is('input[type="checkbox"]') ||
                            $(ev.target).closest('.o_list_record_selector').length) {
                            return;
                        }

                        // Nếu click vào button, không mở form
                        if ($(ev.target).is('button') || $(ev.target).closest('button').length) {
                            return;
                        }
                        ...
                        // Gọi hàm parent để mở form
                        var result = this._super.apply(this, arguments);

                        // Reset flag after a delay
                        setTimeout(function() {
                            self._isFormOpen = false;
                        }, 500);
