...
class RealEstateSearchExpired(models.Model):
    _name = 'real.estate.search.expired'
    _description = 'Real Estate Search Expired'
    ...
    source_image = fields.Selection([('newspaper', 'Báo'),
                                    ...
                                    string='Nguồn tìm về')
    type_estate_ids = fields.Many2many('type.estate', 'real_estate_search_expired_type_estate_rel', string='Loại')
    group_style_id = fields.Many2one('group.style', string='Nhóm kiểu')
    style_ids = fields.Many2many('estate.style', 'real_estate_search_expired_estate_style_rel', string='Kiểu')
    type_demand_ids = fields.Many2many('type.demand', 'real_estate_search_expired_type_demand_rel', string='Nhu cầu')
    secondary_form_ids = fields.Many2many('secondary.form', 'real_estate_search_expired_secondary_form_rel',
                                          string='Hình thức phụ')
    structure_ids = fields.Many2many('estate.structure', 'real_estate_search_expired_estate_structure_rel',
                                     string='Cấu trúc')
    job_profession_ids = fields.Many2many('job.profession', 'real_estate_search_expired_job_profession_rel',
                                          string='Ngành nghề')
    group_direction_id = fields.Many2one('group.direction', string='Nhóm hướng')
    way_ids = fields.Many2many('estate.way', 'real_estate_search_expired_estate_way_rel', string='Lối đi')

    real_estate_ids = fields.Many2many('real.estate', 'real_estate_search_expired_real_estate_rel', string='Nhà đất')
    type_number = fields.Selection([('even_number', 'Số chẵn'),
                                    ('odd_number', 'Số lẽ')], string='Kiểu số nhà')

    def action_search_real_estate(self):
        for record in self:
            estate_ids = record.get_real_estate_ids()
            real_estates = record.env['real.estate'].browse(estate_ids)
            real_estates_exp = real_estates.filtered(lambda estate: estate.is_expired == True)
            record.real_estate_ids = [(6, 0, real_estates_exp.ids)]
